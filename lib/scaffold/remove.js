...
/**
 * Will remove a service from myntcore-node.json
...
/**
 * Will remove the Node.js service and from the myntcore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The myntcore-node configuration directory
...
function remove(options, done) {
  ...
  var myntcoreConfigPath = path.resolve(configPath, 'myntcore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(myntcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a myntcore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to myntcore-node.json
        removeConfig(myntcoreConfigPath, service, next);
      });
    }, done
  );
}
