...
var BASE_PACKAGE = {
  description: 'A full Myntcoin node build with Myntcore',
  repository: 'https://github.com/silence48/myntcore',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
...
/**
 * Will create a base Myntore Node configuration directory and files.
 * @param {Object} options
 * @param {String} options.network - "testnet" or "livenet"
 * @param {String} options.datadir - The myntcoin database directory
...
/**
 * Will setup a directory with a Myntore Node directory, configuration file,
 * myntcoin configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the myntcore node configuration directory
 * @param {String} options.datadir - The path to the myntcoin datadir
...
function create(options, done) {
  ...
  async.series([
    ...
    function(next) {
      // Setup the myntcoin directory and configuration
      if (!fs.existsSync(absDataDir)) {
        createMyntoinDirectory(absDataDir, next);
      } else {
        next();
      }
    },
