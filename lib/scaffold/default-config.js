...
/**
 * Will return the path and default myntcore-node configuration. It will search for the
 * configuration file in the "~/.myntcore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.myntcore');
  var defaultConfigFile = path.resolve(defaultPath, './myntcore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['myntd', 'web'];
