 ...
 */
function Myntoin(options) {
  if (!(this instanceof Myntoin)) {
    return new Myntoin(options);
  }
...
util.inherits(Myntoin, Service);

Myntoin.dependencies = [];

Myntoin.DEFAULT_MAX_TXIDS = 1000;
Myntoin.DEFAULT_MAX_HISTORY = 50;
Myntoin.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Myntoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Myntoin.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Myntoin.DEFAULT_SPAWN_RESTART_TIME = 5000;
Myntoin.DEFAULT_SPAWN_STOP_TIME = 10000;
Myntoin.DEFAULT_TRY_ALL_INTERVAL = 1000;
Myntoin.DEFAULT_REINDEX_INTERVAL = 10000;
Myntoin.DEFAULT_START_RETRY_INTERVAL = 5000;
Myntoin.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Myntoin.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Myntoin.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Myntoin.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Myntoin.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Myntoin.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || Myntoin.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Myntoin.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Myntoin.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Myntoin.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Myntoin.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Myntoin.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Myntoin.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Myntoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Myntoin.prototype._initCaches = function() {
...
Myntoin.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Myntoin.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Myntoin.prototype.getPublishEvents = function() {
  return [
    {
...
Myntoin.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'myntd/' + name, 'total:', this.subscriptions[name].length);
};

Myntoin.prototype.unsubscribe = function(name, emitter) {
...
Myntoin.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
...
Myntoin.prototype.unsubscribeBalance = function(emitter, addresses) {
    var self = this;
    if(!addresses) {
...
Myntoin.prototype.unsubscribeBalanceAll = function(emitter) {
...
Myntoin.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
...
Myntoin.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
 ...
 */
Myntoin.prototype.unsubscribeAddressAll = function(emitter) {
...
Myntoin.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Myntoin.DEFAULT_CONFIG_SETTINGS;
...
Myntoin.prototype._parseMMyntinConf = function(configPath) {
  var options = {};
...
Myntoin.prototype._expandRelativeDatadir = function() {
...
Myntoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  log.info('Using myntcoin config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseMyntoinConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseMyntoinConf(networkConfigPath));
  }
...
Myntoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
...
Myntoin.prototype._resetCaches = function() {
...
Myntoin.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Myntoin.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Myntoin.prototype._initChain = function(callback) {
  var self = this;
...
Myntoin.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 7774
  };
  if (this.node.network === myntcore.Networks.testnet) {
    networkOptions.rpcport = 7775;
  }
  return networkOptions;
};

Myntoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === myntcore.Networks.testnet) {
...
Myntoin.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === myntcore.Networks.testnet) {
...
Myntoin.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
...
Myntoin.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Myntoin.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Myntoin Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Myntoin.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Myntoin.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
Myntoin.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
...
Myntoin.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
Myntoin.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
...
Myntoin.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Myntoin.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Myntoin.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Myntoin.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
Myntoin.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Myntoin Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || Myntoin.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

Myntoin.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Myntoin.prototype._stopSpawnedMMyntin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned myntcoin process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean myntcoin process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Myntoin.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedMyntoin(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting myntcoin process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Myntoin process unexpectedly exited with code:', code);
        log.warn('Restarting myntcoin child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Myntoin process restarted');
          });
...
Myntoin.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
 ...
 */
Myntoin.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Myntoin configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Myntoin.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Myntoin.prototype.syncPercentage = function(callback) {
  var self = this;
...
Myntoin.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {Function} callback
 */
Myntoin.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Myntoin.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
Myntoin.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Myntoin.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Myntoin.prototype._getHeightRangeQuery = function(options, clone) {
 ...
 * @param {Function} callback
 */
Myntoin.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
Myntoin.prototype._getConfirmationsDetail = function(transaction) {
...
Myntoin.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Myntoin.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Myntoin.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A myntcoin transaction id
 * @param {Function} callback
 */
Myntoin.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Myntoin.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
Myntoin.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
Myntoin.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
...
+ * @param {Function} callback
+ */
Myntoin.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Myntoin.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
Myntoin.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Myntoin.prototype.getRawBlock = function(blockArg, callback) {
...
Myntoin.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Myntoin.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a Myntore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
Myntoin.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
Myntoin.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Myntoin.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Myntoin.prototype.estimateFee = function(blocks, callback) {
  var self = this;
...
Myntoin.prototype.estimateSmartFee = function(blocks, conservative, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Myntoin.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Myntoin.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
+ * @param {Function} callback
+ */
Myntoin.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Myntore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Myntoin.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Myntoin.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Myntoin.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Myntoin.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Myntoin.prototype.getInfo = function(callback) {
  var self = this;
...
Myntoin.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Myntoin.prototype.stop = function(callback) {
...
Myntoin.prototype.getBlockReward = function(height) {
  var nSubsidy = 250 * 1e8;
  var SwapCoins = 1;
  var SwapPrep = 2;
  var PostSwap = 50;
  var FirstHalving = 1647840;
  var SecondHalving = 3750240;
  var ThirdHalving = 5852640;
  var FourthHalving = 7955040;
  var FinalHalving = 10057440;

  if (height == SwapCoins) { nSubsidy = 114000000 * 1e8; }
  if (height >= SwapPrep && height <= PostSwap) { nSubsidy = 0 * 1e8; }
  if (height >= PostSwap && height <= FirstHalving) { nSubsidy = 250 * 1e8; }
  if (height >= FirstHalving && height <= SecondHalving) { nSubsidy = 125 * 1e8; }
  if (height >= SecondHalving && height <= ThirdHalving) { nSubsidy = 62.5 * 1e8; }
  if (height >= ThirdHalving && height <= FourthHalving) { nSubsidy = 31.25 * 1e8; }
  if (height >= FourthHalving && height <= FinalHalving) { nSubsidy = 15.625 * 1e8; }
  if (height >= FinalHalving) { nSubsidy = 7.8125 * 1e8; }


  return parseInt(nSubsidy.toString(10)); 
};
 ...
 * @param {Function} callback
 */
Myntoin.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @return {*}
 */
Myntoin.prototype.getMiningInfo = function(callback) {
    var self = this;
...
Myntoin.prototype.getNetworkHash = function(height, callback) {
    var self = this;
...
Myntoin.prototype.getPeerInfo = function(callback) {
    var self = this;
